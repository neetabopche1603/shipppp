            ...
            <table class="table text-nowrap" id="clientTable">
                <thead>
                    <tr>
                        ...
                        <th class="border-top-0">Pickup Review</th>
                ...
                <tbody>
                         @foreach($data as $row)
                            ...
                            <form method="POST" action="{{ route('admin.deleteBooking', $row->id) }}">
                                @csrf
                                <a href="" class="show_confirm"><i class="fa fa-trash-alt"></i></a> &nbsp; <a data-toggle="modal" data-target="#itemmodal" id="{{ $row->uid }}" class="Imodal"><i class="fa fa-eye"></i></a>
                            </form>
