...
// Function To update Coupons
    ...
    public function createplans()
    {
        ...
        $sfdata = DB::table('planfeatures')->select('*')->get();
        return view('admin.createplan', compact('img','sfdata'));
    }
    ...
    public function createplan(Request $request)
    {
        ...
        $request->validate([
            ...
            // $plan_features =''; 
            // if(!empty($request->plan_features)){
            //     foreach ($request->plan_features as $value) {
            //     $plan_features.= $value.',';
            // }
            // $pfeatures = rtrim($plan_features, ',');
            // }else{
            //     $pfeatures ='';
            // }

            $pfeatures = '1,2,3,4,5,6,7,9';

            Stripe::setApiKey(env('STRIPE_SECRET'));
            $currency = env('CURRENCY');
            if($request->interval!="free"){
                $plancreateres = \Stripe\Plan::create(array(
                    "product" => [
                        "name" => $request->name
                    ],
                    "amount" => $price * 100,
            ...
            $splan->name = $request->name;
            $splan->intervals = $request->interval;
            $splan->price = $price;
            $splan->duration = $duration;
            $splan->stripe_plan_id = isset($plancreateres->id) ? $plancreateres->id : 0;
            $splan->stripe_product_id = isset($plancreateres->product) ? $plancreateres->product : 0;
            $splan->plan_features = $pfeatures;
            //$splan->users = json_encode($users);
            $splan->save();
        return redirect()->route('admin.viewPlans');  
    }


    public function viewPlan()
    {
        $data = Subscriptionplan::whereNull('deleted_at')->orderBy('id','asc')->get();
        $image = User::select('profileimage')->where('is_admin','=',1)->get();
        $img = json_decode($image);
        return view('admin.viewPlans', compact('data','img'));
    }
    ...
    public function updateplan($id)
    {
        ...
        $img = json_decode($image);
        $sfdata = DB::table('planfeatures')->select('*')->get();
        return view('admin.editPlan', compact('img','data','sfdata'));
    }
...
// Function To update Coupons
    ...
    public function updatePlans(Request $request)
    {
        ...
        ->update([
            ...
            'duration'     => $duration,
            'stripe_plan_id'     => isset($plancreateres->id) ? $plancreateres->id : 0,
            'stripe_product_id' => isset($plancreateres->product) ? $plancreateres->product : 0,
            'plan_features' => $pfeatures
        ]);
...
// Function to Save Time into DB
    ...
    {
        DB::enableQueryLog();
        $data = DB::table('subscription_details')
              ->join('shipments', 'shipments.id','=', 'subscription_details.user_id','left')
              //->where('bookings.status','!=','Not Confirmed')
              ->select('subscription_details.id','shipments.name','shipments.lname','shipments.email','shipments.phone','subscription_details.plan_interval','subscription_details.plan_type','subscription_details.amount')
              ->orderBy('subscription_details.id','DESC')
              ->get();
        //dd(DB::getQueryLog());
        $image = User::select('profileimage')->where('is_admin','=',1)->get();
        $img = json_decode($image);
        //echo "<pre>";     print_r($data); die();
        return view('admin.transactions', compact('data','img'));
    }
    ...
    public function viewTransactionDetails($id)
    {
        $data = DB::table('subscription_details')
              ->join('shipments', 'shipments.id','=', 'subscription_details.user_id','left')
              ->where('subscription_details.id',$id)
              ->select('shipments.name','shipments.companyname','shipments.lname','shipments.email','shipments.phone','subscription_details.*')
              ->first();
           //   echo "<pre>";                print_r($data); die;
        $image = User::select('profileimage')->where('is_admin','=',1)->get();
        $img = json_decode($image);
        return view('admin.transactionDetails', compact('data','img'));
    }
